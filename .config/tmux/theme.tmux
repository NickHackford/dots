...
set -g status-right "#[fg=$thm_black,bg=$thm_bg,nobold,nounderscore,noitalics]$TR#[fg=$thm_blue,bg=$thm_black] #{?client_prefix,#[fg=$thm_orange],#[fg=$thm_cyan]}󰄚 #[fg=$thm_grey,bg=$thm_bg,nobold,nounderscore,noitalics]$TR#[fg=$thm_black,bg=$thm_grey] %Y-%m-%d $BS %I:%M %p #[fg=$thm_blue,bg=$thm_grey,nobold,nounderscore,noitalics]$TR#[fg=$thm_black,bg=$thm_blue,bold] #h "
