...
Variants {
    id: variants
    // Prefer monitor1, fallback to monitor2 if not available
    // (run: hyprctl monitors -j | jq -r '.[].name' to see all)
    model: {
        const m1 = Quickshell.screens.filter(s => s.name === NixConfig.monitor1Name);
        if (m1.length > 0)
            return m1;
        const m2 = Quickshell.screens.filter(s => s.name === NixConfig.monitor2Name);
        if (m2.length > 0)
            return m2;
        return Quickshell.screens;
    }

    // Expose menu state for other components (like notifications)
    property bool menuOpen: variants.instances.length > 0 ? variants.instances[0].menuOpen : false
